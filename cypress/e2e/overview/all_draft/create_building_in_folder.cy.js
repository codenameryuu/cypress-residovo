...
describe("Save Data Spec", () => {
  ...
  it("Should save data successfully with valid data", () => {
    ...
    // * Click first draft under subcategory building create
    cy.get('.ag-center-cols-container .ag-row a[href*="/subcategory/"][href*="/building/create"]').should("have.length.at.least", 1).first().click();
    cy.url().should("match", /\/dashboard\/category\/[^/]+\/subcategory\/[^/]+\/building\/create/);
    cy.wait(3000);
