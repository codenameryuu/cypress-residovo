...
describe("Create Damage Report Spec", () => {
  beforeEach(() => {
    cy.login();
  });

  it("Should create damage report successfully", () => {
    ...
    // * Intercept create damage report API
    cy.intercept("POST", "**/api/v1/dashboard/damage-report").as("createDamageReport");

    // * Click on confirm button
    cy.get("#updateAccountConfirmDamageReport button.upload-button-next").should("exist").and("not.be.disabled").click();
    cy.wait(1000);

    // * Wait for create damage report API to be called
    cy.wait("@createDamageReport")
      .its("response")
      .should((response) => {
